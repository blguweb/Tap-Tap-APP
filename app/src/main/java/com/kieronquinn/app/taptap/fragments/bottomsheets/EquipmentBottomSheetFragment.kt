...
import com.kieronquinn.app.taptap.fragments.equipment.EquipmentListFragment
...
import com.kieronquinn.app.taptap.fragments.SettingsCalibrationFragment
...
class EquipmentBottomSheetFragment : BottomSheetDialogFragment(), NavController.OnDestinationChangedListener {

    companion object {
        private const val REQUEST_CODE_SELECT_APP = 1001
    }

    private var storedEquipment: EquipmentInternal? = null
    private var storedEquipmentCallback: ((EquipmentInternal) -> Unit)? = null
    private var stringOne : String = "请从第一个位置指示设备位置并Tap Tap"
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        return inflater.inflate(R.layout.fragment_bottomsheet_equipment, container, false)
    }
    ...
    private fun trySetupFragment(){
        val equipmentListFragment = navHostFragment.childFragmentManager.fragments.firstOrNull() as? EquipmentListFragment
        equipmentListFragment?.let {
            ...
            it.setItemClickListener { equipment ->
                TTS(getActivity() as Activity, stringOne, true)
                mBluetooth.startcalibration = 1
                mBluetooth.equipment = equipment.action.name
                val dataType = equipment.action.dataType
                if(dataType != null){
                    //Fire off to data picker
                    launchDataPicker(equipment, dataType){ completedGate ->
                        val bundle = Bundle()
                        bundle.putParcelable(SettingsCalibrationFragment.addResultKey, completedGate)
                        setFragmentResult(SettingsCalibrationFragment.addResultKey, bundle)
                        dismiss()
                    }
                }else {
                    val bundle = Bundle()
                    bundle.putParcelable(SettingsCalibrationFragment.addResultKey, equipment)
                    setFragmentResult(SettingsCalibrationFragment.addResultKey, bundle)
                    dismiss()
                }
            }
    ...
    private fun launchDataPicker(equipment: EquipmentInternal, dataTypes: EquipmentDataTypes, callback: (EquipmentInternal) -> Unit){
        storedEquipmentCallback = callback
        storedEquipment = equipment
        when(dataTypes){
            EquipmentDataTypes.PACKAGE_NAME -> {
            ...
            override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
                ...
                if(requestCode == REQUEST_CODE_SELECT_APP && resultCode == Activity.RESULT_OK){
                    val currentEquipment = storedEquipment ?: return
            val packageName = data?.getStringExtra(AppsFragment.KEY_SELECTED_APP)
            if(packageName.isNullOrEmpty()) return
            currentEquipment.data = packageName
            storedEquipmentCallback?.invoke(currentEquipment)
            storedEquipmentCallback = null
